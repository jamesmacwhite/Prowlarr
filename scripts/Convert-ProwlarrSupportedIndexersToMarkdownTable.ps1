...
function Invoke-SearchReplace
{
    ...
    if ($SearchValue)
    {
        foreach ($dict in $SearchTypes)
        {
            $SearchValue = $SearchValue -replace $dict.Name, $dict.Value
        }
        return $SearchValue
    }
...
function Invoke-ConvertToMarkDownTable
{
    ...
    return $($object | Format-MarkdownTableTableStyle Indexer, Description, Language, 'Supports Raw Search', 'Search Types', 'TV Search Types', 'Movie Search Types', 'Music Search Types', 'Book Search Types', -HideStandardOutput -ShowMarkdown -DoNotCopyToClipboard )
}
...
function Invoke-IndexerDetailName
{
    ...
    )
    {
        if ($indexer.protocol -eq 'usenet')
        {
            ...
            return { Invoke-IndexerDetailNameUse($indexer) }
        }
        else
        {
            return { Invoke-IndexerDetailNameTor($indexer) }
        }
    }
}
## End Variables
...
$indexer_tbl_obj = @{}
## ToDo Fix this; Indexer is ... as is Description. Search Types are NULL
foreach ($indx in $indexer_obj)
{
    $indexer_tbl_obj = [ordered]@{
        Indexer               = $(IF ($indx.IndexerUrls) { '[' + ($indx.name -replace $markdown_escape_regex, $markdown_escape_regex_rep) + '](' + ($indx.IndexerUrls[0]) + ')' + '{#' + $indx.infoLink.Replace($wiki_infolink.ToString(), '') + '}' } Else { IF ($indx.fields.value[0]) { '[' + ($indx.name -replace $markdown_escape_regex, $markdown_escape_regex_rep) + '](' + ($indx.fields.value[0].Replace('api.', '').Replace('feed.', '')) + ')' + '{#' + $indx.infoLink.Replace($wiki_infolink.ToString(), '') + '}' } Else { ($indx.name -replace $markdown_escape_regex, $markdown_escape_regex_rep) + '{#' + $indx.infoLink.Replace($wiki_infolink.ToString(), '') + '}' } } )
...
$tbl_fmt_PubUse = Invoke-ConvertToMarkDownTable($tbl_PubUse)
$tbl_fmt_PrvUse = Invoke-ConvertToMarkDownTable($tbl_PrvUse)
$tbl_fmt_PubTor = Invoke-ConvertToMarkDownTable($tbl_PubTor)
$tbl_fmt_PrvTor = Invoke-ConvertToMarkDownTable($tbl_PrvTor)
